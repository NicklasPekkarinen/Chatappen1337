                ...
                for(DataSnapshot snapshot : dataSnapshot.getChildren()){
                    ...
                    for (int i = 0; i < groupChatInfo.getReceivers().size(); i++) {
                        for (int j = 0; j < userIds.size(); j++) {
                            if (groupChatInfo.getReceivers().get(i).equals(myId) && groupChatInfo.getSender().equals(userIds.get(j))
                            || groupChatInfo.getReceivers().get(i).equals(userIds.get(j)) && groupChatInfo.getSender().equals(myId)) {
                                if (!chat.contains(groupChatInfo)) {
